...
export default function TradeHistory() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-6 pb-8">
      ...
      {history.data?.length ? <section className="overflow-hidden rounded-2xl border border-[#2a2a30] bg-[#1a1a1e]"><div className="overflow-x-auto"><table className="w-full min-w-[1160px] text-left text-sm"><thead className="bg-[#222226] text-[10px] uppercase tracking-[0.12em] text-[#9a9aa2]"><tr><th className="px-4 py-3">Close Time (IST)</th><th className="px-4 py-3">Underlying</th><th className="px-4 py-3">CE / PE</th><th className="px-4 py-3">Lots</th><th className="px-4 py-3">Entry</th><th className="px-4 py-3">Exit</th><th className="px-4 py-3">P&amp;L USD</th><th className="px-4 py-3">Fees INR</th><th className="px-4 py-3">Net INR</th><th className="px-4 py-3">Note</th></tr></thead><tbody className="divide-y divide-[#2a2a30]">{history.data.map(trade => { const underlying = underlyingFromOptionSymbol(trade.ceSymbol); return <tr key={trade.id} className="text-[#e8e8ea]"><td className="whitespace-nowrap px-4 py-4 font-mono text-xs text-[#9a9aa2]">{formatIstDateTime(trade.closedAt)}</td><td className="px-4 py-4 text-xs font-medium">{underlying ? getUnderlyingDetails(underlying).monitorLabel : "—"}</td><td className="px-4 py-4"><p className="font-mono text-xs">{trade.ceSymbol}</p><p className="mt-1 font-mono text-xs text-[#9a9aa2]">{trade.peSymbol}</p></td><td className="px-4 py-4 tabular-nums">{trade.lots}</td><td className="px-4 py-4 font-mono text-xs">${Number(trade.ceEntry).toFixed(2)} / ${Number(trade.peEntry).toFixed(2)}</td><td className="px-4 py-4 font-mono text-xs">${Number(trade.ceExit).toFixed(2)} / ${Number(trade.peExit).toFixed(2)}</td><td className="px-4 py-4 tabular-nums">${Number(trade.pnlUsd).toFixed(2)}</td><td className="px-4 py-4 tabular-nums text-[#9a9aa2]">{money(trade.feesInr)}</td><td className={`px-4 py-4 font-semibold tabular-nums ${Number(trade.netInr) >= 0 ? "text-[#3ddc84]" : "text-[#ff6b6b]"}`}>{money(trade.netInr)}</td><td className="max-w-xs px-4 py-4 text-xs leading-5 text-[#9a9aa2]">{trade.note ?? "—"}</td></tr>; })}</tbody></table></div></section> : null}
